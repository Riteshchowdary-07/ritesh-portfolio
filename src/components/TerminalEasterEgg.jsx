...
import { personalData, skillsCategories, projectsData } from '../data/portfolioData';
...
export const TerminalEasterEgg = ({ isOpen, onClose }) => {
  ...
  const handleCommand = (e) => {
    ...
    if (lowerCmd === 'sudo hire ritesh') {
      ...
      newHistory.push({
        text: 'Skills: Python, Java, C++, KNN, Decision Trees, K-Means, HTML, CSS, JS, Flask, DSA, UI/UX',
        type: 'info'
      });
    } else if (lowerCmd === 'projects') {
      newHistory.push({
        text: 'Projects: 1. AI Career Counselor (Gemini API & Flask)  2. AI Meme Generator',
        type: 'info'
      });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-fadeIn">
      <div className="w-full max-w-2xl bg-[#090d16] border border-cyan-500/50 rounded-2xl overflow-hidden shadow-2xl font-mono text-xs text-slate-200">
        
        {/* Terminal Header */}
        <div className="bg-[#0f1524] px-4 py-3 border-b border-white/10 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="w-3 h-3 rounded-full bg-red-500/80"></span>
            <span className="w-3 h-3 rounded-full bg-amber-500/80"></span>
            <span className="w-3 h-3 rounded-full bg-emerald-500/80"></span>
            <span className="ml-2 text-slate-400 flex items-center gap-1.5 text-[11px]">
              <TerminalIcon className="w-3.5 h-3.5 text-cyan-400" /> ritesh@dev-environment:~
            </span>
          </div>
        ...
        {/* Terminal Screen */}
        <div className="p-4 h-72 overflow-y-auto space-y-2 bg-[#05070c]">
        ...
        {/* Command Form */}
        <form onSubmit={handleCommand} className="p-3 bg-[#0d1220] border-t border-white/10 flex items-center gap-2">
          ...
          />
          <button type="submit" className="px-3 py-1 bg-cyan-950 text-cyan-400 border border-cyan-500/30 rounded text-[11px]">
            Run
          </button>
        </form>
